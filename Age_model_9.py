...
def res_block(inputs, expansion, strides, alpha, filters, weight_decay, block_id, blocks="depth"):
    ...
    if input_channels == pointwise_filters and strides == 1:
        ...
        return tf.keras.layers.Multiply(name=prefix + "multiply")([inputs, h])
    return h

def My_compact_model(input_shape=(224, 224, 3), num_classes=60, weight_decay=1e-4):
    ...
    h = tf.keras.layers.Conv2D(filters=32,
                               ...
                               kernel_regularizer=tf.keras.regularizers.l2(weight_decay),
                               name="Conv1")(h)
    h = tf.keras.layers.BatchNormalization(name="Conv1_bn")(h)
    h = tf.keras.layers.ReLU(6., name="Conv1_relu")(h)

    h = res_block(inputs=h, expansion=3, strides=2, alpha=1.0, filters=32, weight_decay=weight_decay, block_id=1, blocks='depth')
    h = res_block(inputs=h, expansion=3, strides=1, alpha=1.0, filters=32, weight_decay=weight_decay, block_id=2, blocks='depth')
    h = res_block(inputs=h, expansion=3, strides=1, alpha=1.0, filters=32, weight_decay=weight_decay, block_id=3, blocks='depth')

    h = res_block(inputs=h, expansion=4, strides=2, alpha=1.0, filters=64, weight_decay=weight_decay, block_id=4, blocks='depth')
    h = res_block(inputs=h, expansion=4, strides=1, alpha=1.0, filters=64, weight_decay=weight_decay, block_id=5, blocks='depth')
    h = res_block(inputs=h, expansion=4, strides=1, alpha=1.0, filters=64, weight_decay=weight_decay, block_id=6, blocks='depth')
    h = res_block(inputs=h, expansion=4, strides=1, alpha=1.0, filters=64, weight_decay=weight_decay, block_id=7, blocks='depth')

    h = res_block(inputs=h, expansion=5, strides=2, alpha=1.0, filters=96, weight_decay=weight_decay, block_id=8, blocks='depth')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=96, weight_decay=weight_decay, block_id=9, blocks='depth')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=96, weight_decay=weight_decay, block_id=10, blocks='depth')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=96, weight_decay=weight_decay, block_id=11, blocks='depth')

    h = res_block(inputs=h, expansion=5, strides=2, alpha=1.0, filters=128, weight_decay=weight_decay, block_id=12, blocks='depth')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=128, weight_decay=weight_decay, block_id=13, blocks='groupconv')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=128, weight_decay=weight_decay, block_id=14, blocks='groupconv')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=128, weight_decay=weight_decay, block_id=15, blocks='groupconv')
    h = res_block(inputs=h, expansion=5, strides=1, alpha=1.0, filters=128, weight_decay=weight_decay, block_id=16, blocks='groupconv')

    h = tf.keras.layers.Conv2D(filters=1280,
                               kernel_size=1,
                               strides=1,
                               padding='same',
                               use_bias=False,
